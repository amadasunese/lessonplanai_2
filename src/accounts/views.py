...
from src.accounts.models import User, Tutor
from .forms import LoginForm, RegisterForm, ContactForm, ResetPasswordForm, ResetPasswordRequestForm
...
###############################################
#   User confirmation and activation routes    #
###############################################
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        flash('Allowed file types are: {}'.format(', '.join(ALLOWED_EXTENSIONS)))
        return redirect(request.url)
...
@accounts_bp.route('/display-photo/<int:tutor_id>')
def display_photo(tutor_id):
    tutor = Tutor.query.get_or_404(tutor_id)
    response = send_file(io.BytesIO(tutor.photo_data), mimetype='image/jpeg', as_attachment=False)
    return response